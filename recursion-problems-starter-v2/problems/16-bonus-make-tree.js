...
const makeTree = (categories, parent) => {
  ...
  let object = categories.filter(elem => {
    return elem.parent === parent
  })
  object.forEach(elem =>{
    let keyobject = elem
    let index = categories.indexOf(keyobject)
    if(keyobject === undefined) return {}
    categories.splice(index,1)
    let valobject = categories.filter(elem => {
      return elem.parent === keyobject.id
    })
    valobject.forEach(elem => {
      result[keyobject.id] = elem.id
    })
    parent = keyobject.id
    result[keyobject.id] = makeTree(categories, parent)
  })
  return result
}
